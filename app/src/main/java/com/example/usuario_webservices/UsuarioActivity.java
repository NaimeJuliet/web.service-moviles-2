...
public class UsuarioActivity extends AppCompatActivity implements Response.Listener<JSONObject>,Response.ErrorListener{
    ...
    Button jbtregistrar,jbtregresar,jbtconsultar,jbteliminar,jbtmodificar;
    RequestQueue rq;
    JsonRequest jrq;
    ...
    private void registrar_usuario() {
        String url = "http://172.16.60.25:8081/usuarios/registrocorreo.php";
    ...
    public void Modificar_usuario(){
        String url = "http://172.16.60.25:8081/usuarios/actualiza.php";
        StringRequest postRequest = new StringRequest(Request.Method.POST, url,
                new Response.Listener<String>()
                {
                    @Override
                    public void onResponse(String response) {
                        Limpiar_campos();
                        Toast.makeText(getApplicationContext(), "Registro de usuario modificado correctamente!", Toast.LENGTH_LONG).show();
                    }
                },
                new Response.ErrorListener()
                {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(getApplicationContext(), "Modificacion de usuario incorrecto!", Toast.LENGTH_LONG).show();
                    }
                }
        ) {
            @Override
    ...
    public void Eliminar_usuario(){
        String url = "http://172.16.60.25:8081/usuarios/elimina.php";
        StringRequest postRequest = new StringRequest(Request.Method.POST, url,
                new Response.Listener<String>()
                {
                    @Override
                ...
                new Response.ErrorListener()
                {
                    @Override
                    public void onErrorResponse(VolleyError error) {
                        Toast.makeText(getApplicationContext(), "Eliminacion de usuario incorrecta!", Toast.LENGTH_LONG).show();
                    }
                }
        ) {
            @Override
    ...
    public void consultar_usuario(){
        if (jetusr.getText().toString().isEmpty()){
            ...
            String url = "http://172.16.60.25:8081/usuarios/consulta.php?usr="+jetusr.getText().toString()+"";
